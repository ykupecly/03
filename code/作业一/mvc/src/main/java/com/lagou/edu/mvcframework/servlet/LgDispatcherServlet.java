...
public class LgDispatcherServlet extends HttpServlet {
    ...
    /**
     * 处理自定义注解@Security（哪些用户拥有访问该Handler方法的权限）
     */
    private String[] doSecurity() {
        if (ioc.isEmpty()) {
            return null;
        }
...
//            }
            ...
            if (aClass.isAnnotationPresent(LagouController.class) && aClass.isAnnotationPresent(LagouSecurity.class)) {
                LagouSecurity annotation = aClass.getAnnotation(LagouSecurity.class);
                userNameArray = annotation.value();
            }
...
//        method.invoke() //
        ...
        String[] userNameArray = doSecurity();//获取@Security中定义的有权限的用户
        //有访问权限则放行，没有访问权限在页面上输出
        if (!ArrayUtils.contains(userNameArray, paraValues[2])) {
